...
class _MiniPlayerState extends State<MiniPlayer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    )..repeat(); // Start the animation by default

    // Listen to the player state to start or stop the animation
    Get.find<PlayerController>().buttonState.listen((state) {
      if (state == PlayButtonState.playing) {
        _controller?.repeat(); // Resume rotation when playing
      } else if (state == PlayButtonState.paused) {
        _controller?.stop(); // Stop rotation when paused
      }
    });
  }
